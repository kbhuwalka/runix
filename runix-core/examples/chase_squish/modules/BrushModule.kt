...
object BrushModule {
    ...
    object Monitors {
        val TemperatureWarningMonitor = monitor("BrushTemperatureHigh") {
            dependsOn(Conditions.isTemperatureAbove80)
            fireIf(Conditions.isTemperatureAbove80.asCondition("BrushTemperatureAbove80"))
            emits(Signals.BrushTemperatureHigh)
        }

        val BrushOverheatMonitor = monitor("BrushOverheat") {
            dependsOn(Conditions.isTemperatureAbove100)
            fireIf(Conditions.isTemperatureAbove100.persistedFor(3.seconds, "BrushTempAbove100For3Seconds"))
